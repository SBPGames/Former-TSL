...
PROJECT_ASSETS_ROOT = "sevenlives/assets"

def getAssetFolder(subpackage: str, element: str = None):
    assert any(
        [subpackage == d for d in filter(lambda f: f != "__pycache__", os.listdir(PROJECT_ASSETS_ROOT))]
    ), "You have to give a real subpackage name."

    spRoot = os.path.join(PROJECT_ASSETS_ROOT, subpackage)
